...
class VehicleModel(models.Model):
    ...
    description = models.CharField(
        max_length=200, verbose_name='Описание', null=True, blank=True)
...
class Car(models.Model):
    '''Машина'''
    serialNumberCar = models.CharField(
        max_length=10, unique=True, verbose_name='Зав. № машины')
    vehicleModel = models.ForeignKey(
        VehicleModel, on_delete=models.CASCADE, verbose_name='Модель техники')
    ...
    serialNumberEngine = models.CharField(
        max_length=10, verbose_name='Зав № двигателя')
    transmissionModel = models.ForeignKey(
        TransmissionModel, on_delete=models.CASCADE, verbose_name='Модель трансмиссии')
    serialNumberTransmission = models.CharField(
        max_length=10, verbose_name='Зав. № трансмиссии')
    driveAxleModel = models.ForeignKey(
        DriveAxleModel, on_delete=models.CASCADE, verbose_name='Модель ведущего моста')
    serialNumberDriveAxle = models.CharField(
        max_length=10, verbose_name='Зав № ведущего моста')
    steeringAxleModel = models.ForeignKey(
        SteeringAxleModel, on_delete=models.CASCADE, verbose_name='Модель управляемого моста')
    serialNumberSteeringAxle = models.CharField(
        max_length=10, verbose_name='Зав № управляемого моста')
    supplyContract = models.CharField(
        max_length=20, verbose_name='Договор поставки №, дата')
    shippingDate = models.DateField(
        verbose_name='Дата отгрузки с завода', null=False, blank=False)
    consignee = models.CharField(
        max_length=300, verbose_name='Грузополучатель (конечный потребитель)')
    deliveryAddress = models.CharField(
        max_length=300, verbose_name='Адрес поставки (эксплуатации)')
    equipment = models.CharField(
        max_length=20, verbose_name='Комплектация (доп. опции)')
    client = models.ForeignKey(
        ClientProfile, on_delete=models.CASCADE, verbose_name='Клиент')
    serviceCompany = models.ForeignKey(
        ServiceCompanyProfile, on_delete=models.CASCADE, verbose_name='Сервисная компания')
...
class Maintenance(models.Model):
    '''ТО'''
    typeOfMaintenance = models.ForeignKey(
        TypeOfMaintenance, on_delete=models.CASCADE, verbose_name='Вид ТО')
    dataOfMaintenance = models.DateField(
        verbose_name='Дата проведения ТО', null=False, blank=False)
    operatingTime = models.CharField(
        max_length=20, verbose_name='Наработка, м/час')
    workOrderNumber = models.CharField(
        max_length=20, verbose_name='№ заказ-наряда')
    workOrderDate = models.DateField(
        verbose_name='Дата заказ-наряда', null=False, blank=False)
    organizationOfMaintenance = models.ForeignKey(
        OrganizationOfMaintenance, on_delete=models.CASCADE, verbose_name='Организация, проводившая ТО')
    car = models.ForeignKey(
        Car, on_delete=models.CASCADE, verbose_name='Машина')
    serviceCompany = models.ForeignKey(
        ServiceCompanyProfile, on_delete=models.CASCADE, verbose_name='Сервисная компания')
...
class Complaint(models.Model):
    '''Рекламации'''
    dateOfRefusal = models.DateField(
        verbose_name='Дата отказа', null=False, blank=False)
    operatingTime = models.CharField(
        max_length=20, verbose_name='Наработка, м/час')
    failureNode = models.ForeignKey(
        FailureNode, on_delete=models.CASCADE, verbose_name='Узел отказа')
    ...
    sparePartsUsed = models.TextField(
        verbose_name='Используемые запасные части')
    restoreDate = models.DateField(
        verbose_name='Дата восстановления', null=False, blank=False)
    car = models.ForeignKey(
        Car, on_delete=models.CASCADE, verbose_name='Машина')
